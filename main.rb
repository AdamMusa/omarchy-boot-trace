...
class SuiteBackend
  ...
  def initialize(state_dir: File.expand_path("~/.local/state/omarchy-boot-trace"), runner: nil)
    @state_dir = state_dir
  ...
  def scan_system
    overview = command_text(["systemd-analyze", "time"])
    blame = command_text(["systemd-analyze", "blame", "--no-pager"])
    to_ms = lambda do |token|
      value = token.to_s
      if value.end_with?("ms")
        value.sub("ms", "").to_f
      elsif value.end_with?("s")
        value.sub("s", "").to_f * 1_000
      elsif value.end_with?("min")
        value.sub("min", "").to_f * 60_000
      else
        0.0
      end
    end
    ...
    @history = @history.last(MAX_HISTORY)
    @score = total.round
    @summary = total.positive? ? "#{total.round(1)}s last boot" : "Timing unavailable"
    rows
  end
